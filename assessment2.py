...
def hometown(town_name): 
    """Determines if the hometown is Cleveland or not
    ...
    return town_name == 'Cleveland'

def first_and_last(first_name, last_name):
    """Takes in first and last names as arguments and concatenates them
    """

    return first_name + " " + last_name

def greeting(town_name, first_name, last_name):
    if hometown(town_name): 
        print('Hi, ' + first_and_last(first_name,last_name) + ", we're from the same place!")
    else: 
        print('Hi, ' + first_and_last(first_name,last_name) + ", I'd like to visit " + town_name)    
...
def calculate_price(price, state, tax = .05):
    ...
    total_price = price
    if state == 'CA': 
        total_price = total_price * (1 + tax) * (1+.03)
    elif state == 'PA':
        total_price = total_price * (1 + tax) + 2
    elif state == 'MA':
